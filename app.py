...
st.set_page_config(
    page_title="Inclusión Financiera en Latinoamérica",
    page_icon="📈",
    layout="wide",
...
with open("styles1.css") as css_file:
...
# Cargar datos con caché y prepararlos
@st.cache_data
def load():
    df = pd.read_csv("fin.csv", sep=";", encoding="utf-8")
    cols_pct = [
        "account",
        "fiaccount",
        "borrow_money",
...
# Cargar y preparar datos en la aplicación
try:
    df = load()
...
# Aquí puedes añadir el resto de la lógica de tu aplicación Streamlit
st.title("📊 Dashboard de Inclusión Financiera")
...
with tab1:
    st.subheader("📈 Evolución de la inclusión financiera en el tiempo")

    metric_map = {
        "Adultos con cuenta (%)": "account",
        "Cuenta móvil (%)": "mobileaccount",
        "Ahorro total (%)": "ahorro_total",
        "Préstamo de dinero (%)": "borrow_money",
    }
    ...
    plot_bgcolor="#0D2740",         # fondo del área de la gráfica azul
...
with tab2:
    st.subheader("🗺️ Análisis geográfico de la inclusión financiera")
    ...
    geo_metric_map = {
        "Adultos con cuenta (%)": "account",
        "Cuenta en institución financiera (%)": "fiaccount",
        "Cuenta móvil (%)": "mobileaccount",
        "Ahorro total (%)": "ahorro_total",
        "Préstamo de dinero (%)": "borrow_money",
    ...
    fig_geo.update_layout(
        ...
        plot_bgcolor="#0D2740",         # fondo del área de la gráfica
        font_color="#F5F7FA",
        xaxis=dict(
            showgrid=False
        ),
    ...
    st.markdown(f"### 🌎 Mapa de {geo_metric_label} por país")
...
# ================== TAB 3: Top países ================== #
with tab3:
    st.subheader("🏅 Top países por inclusión financiera")
    ...
    with gra2_col1:
        top_n_countries = st.slider(
            "Selecciona cuántos países mostrar (Top N)",
            min_value=5,
            max_value=25,
            value=10,
            step=1
    ...
    fig_countries = px.bar(
        ...
        orientation="h",
        title=f"Top {top_n_countries} países en {country_metric_label}",
    ...
    if df_segment.empty:
        ...
        with gra3_col1:
            fig_seg_account = px.pie(
                ...
                values="account",
                names="group2",
                title=f"Distribución de adultos con cuenta por {segment_label}",
                hole=0.4,
        ...
        with gra3_col2:
            fig_seg_ahorro = px.pie(
                ...
                values="ahorro_total",
                names="group2",
                title=f"Distribución de ahorro total por {segment_label}",
                hole=0.4,
                color_discrete_sequence=pie_colors
            )
